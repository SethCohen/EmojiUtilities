...
export default {
	...
	async execute(interactionCommand) {
		...
		try {
			...
			let data;
			if (nsfw) {
				if (!interactionCommand.channel.nsfw) return await interactionCommand.editReply({ content: 'Sorry, but NSFW content is only allowed in NSFW channels.' });
				...
				.then(async interactionButton => {
					if (interactionButton.customId === 'confirm') {
						...
						interactionCommand.guild.emojis
							.create({ attachment: randomEmoji.image, name: randomEmoji.title })
							.then(emoji => {
