...
async def displaystats(message):
    ...
    try:
        ...
        db_cursor.execute("""SELECT * FROM tableAll ORDER BY occurrence DESC;""")
...
async def on_guild_emojis_update(guild, before, after):
    ...
    if (len(before) > len(after)):
        ...
        try:
            ...
            db_cursor = db_conn.cursor()
            db_cursor.execute("""
                DELETE from tableAll WHERE emoji = ?
                """, (strEmoji,))
...
async def on_guild_join(guild):
    for channel in guild.text_channels:
        if channel.permissions_for(guild.me).send_messages:
            await channel.send(
                ...
                "\nType `ES help` for a list of commands."
    ...
    try:
        ...
        db_cursor = db_conn.cursor()
        db_cursor.execute("""
            CREATE TABLE IF NOT EXISTS tableAll (
            emoji TEXT UNIQUE,
            occurrence INT
            )
            """)

        insert = """INSERT OR IGNORE INTO tableAll(emoji, occurrence) VALUES(?, 0);"""
        ...
        for channel in guild.text_channels:
            if channel.permissions_for(guild.me).send_messages:
                await channel.send("Database created.")
            break
